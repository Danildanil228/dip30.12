...
const revokeAllUserSessions = async (userId) => {
    const result = await pool.query(
        'UPDATE user_sessions SET revoked = true WHERE user_id = $1 AND revoked = false RETURNING id',
        [userId]
    );
...
// Очистка просроченных сессий (можно запускать по расписанию)
