                        ...
                        return (
                            <DropdownMenu key={item.path}>
                                <DropdownMenuTrigger className="flex flex-col items-center justify-center py-1 px-2 flex-1 min-w-0 relative">
                                    ...
                                    <span className={`text-xs mt-1 ${active ? "text-primary font-medium" : "text-muted-foreground"}`}>{item.label}</span>
                                    {active && <motion.div layoutId="mobile-tab-indicator" className="absolute -top-1 w-8 h-1 bg-primary rounded-full" transition={{ type: "spring", stiffness: 500, damping: 30 }} />}
                                </DropdownMenuTrigger>
