        ...
        <div className="lg:hidden fixed bottom-0 left-0 right-0 border-t z-50 bg-background">
            <div className="flex justify-around py-2">
                {filteredNavItems.map((item) => {
                                ...
                                <DropdownMenu>
                                    <DropdownMenuTrigger className="flex flex-col items-center justify-center w-full">
                                        <div className="relative mb-1">
                                            <img src={item.icon} alt={item.label} className={`w-6 h-6 icon ${active ? "filter brightness-0 invert-30 sepia-90 saturate-3000 hue-rotate-220" : "filter brightness-0 opacity-60"}`} />
                                        </div>
                                        <span className={`text-xs ${active ? "text-primary font-medium" : "text-muted-foreground"}`}>{item.label}</span>
                                    </DropdownMenuTrigger>
                                    <DropdownMenuContent side="top" align="center">
                                        {materialItem.map((material) => (
                                            <DropdownMenuItem key={material.path} onClick={() => navigate(material.path)} className={`cursor-pointer ${location.pathname === material.path ? "bg-muted" : ""}`}>
                                                {material.label}
                                            </DropdownMenuItem>
                                        ))}
                                    </DropdownMenuContent>
                    ...
                    return (
                        <Link key={item.path} to={item.path} className="flex flex-col items-center justify-center py-1 px-2 flex-1 min-w-0 relative">
                            <div className="relative mb-1">
                                <img src={item.icon} alt={item.label} className={`w-6 h-6 icon ${active ? "filter brightness-0 invert-30 sepia-90 saturate-3000 hue-rotate-220" : "filter brightness-0 opacity-60"}`} />
                            </div>
                            <span className={`text-xs ${active ? "text-primary font-medium" : "text-muted-foreground"}`}>{item.label}</span>
