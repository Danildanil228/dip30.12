...
export function UserActivityReport() {
    ...
    const { getUserActivityReport, loading } = useReports();
    ...
    const columns = [
        ...
        { key: "role", header: "Роль", width: "120px", format: (v: string) => getRoleBadge(v) },
        { key: "requests_created", header: "Создано заявок", width: "120px", format: (v: number) => formatNumber(v) },
        { key: "requests_approved", header: "Подтверждено", width: "100px", format: (v: number) => <span className="text-green-600">{formatNumber(v)}</span> },
        { key: "requests_rejected", header: "Отклонено", width: "100px", format: (v: number) => <span className="text-red-600">{formatNumber(v)}</span> },
