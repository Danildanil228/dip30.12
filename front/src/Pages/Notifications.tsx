...
export default function Notifications({ onVisited }: LogsProps) {
    ...
    const parseMessageWithLinks = (message: string) => {
        ...
        return parts.map((part, index) => {
            ...
            if (part.type === "user") {
                return (
                    <Link key={index} to={`/profile/${part.id}`} className="underline mx-1">
                        {part.username}
                    </Link>
                );
            }
            if (part.type === "request") {
                return (
                    <Link key={index} to={`/requests/${part.id}`} className="underline mx-1">
                        {part.content}
                    </Link>
                );
            }
            if (part.type === "inventory") {
                return (
                    <Link key={index} to={`/inventories/${part.id}`} className="underline mx-1">
                        {part.content}
                    </Link>
                );
            }
